...
# ===== Color palettes =====
# Six-treatment palette
salinity_palette_6 <- c(
  "ASAN" = "#8DD3C7",  # soft aqua (low salinity)
  "ASEN" = "#4DBBD5",  # teal
  "MSAN" = "#1F78B4",  # blue
  "MSEN" = "#A6CEE3",  # light blue-grey
  "HSAN" = "#FDBF6F",  # warm light orange
  "HSEN" = "#E66101"   # strong orange (high salinity)
)

# Three-level salinity palette (first, third, and fifth colors)
salinity_palette_3 <- c(
  "Ambient" = "#8DD3C7",
  "Low"     = "#1F78B4",
  "High"    = "#FDBF6F"
)
...
```{r data}
################################################################################
##########      FULL MACRO DATA + TAXONOMY PIPELINE (TOLEDO REMOVED)     ########
################################################################################
...
################################################################################
# 4. REMOVE TOLEDO AND PREPARE METADATA + ENVIRONMENTAL DATA
################################################################################

df_meta <- macro %>%
  filter(site != "Toledo") %>%              # REMOVE TOLEDO
  select(site, treatments, salinity, nutrients, sample_code, 
         chlorophyll_a:phosphates, -initial_conductivity) %>%
  mutate(
    ...
    site = factor(site,
                  levels = c("Evora","Porto","Jaca"))  # UPDATED LEVELS
  )
...
################################################################################
# 5. FILTER MACRO DATA BEFORE EXTRACTING COMMUNITY MATRIX
################################################################################

macro_filtered <- macro %>% filter(site != "Toledo")     # REMOVE TOLEDO


################################################################################
# 6. COMMUNITY MATRICES
################################################################################

comm_raw <- macro_filtered %>%
...
################################################################################
# 10. LONG FORMAT COMMUNITY TABLE (TOLEDO REMOVED)
################################################################################

macro_long <- comm_log %>%
  mutate(sample = row_number()) %>%
  bind_cols(df_meta %>% select(salinity, site)) %>%   # already Toledo-free
  pivot_longer(
...
df_bacteria_metadata <- df_bacteria_metadata %>%
  rename(
    ...
    pH            = pH,
    oxygen        = Oxygen.,     # removes trailing dot
    nitrites      = Nitrite,
    phosphates    = PhospJAte
  )

df_bacteria_metadata$Site <- recode(df_bacteria_metadata$Site,
    EV = "Evora",
    JA = "Jaca",
    PO = "Porto"
)

df_bacteria_metadata$Site <- factor(df_bacteria_metadata$Site, levels = c("Evora", "Porto", "Jaca"))
...
# Make sure factors are properly declared
div_df <- div_df %>%
  mutate(
    salinity  = factor(salinity),
    nutrients = factor(nutrients),
...
p_rich_all <- ggplot(div_df, aes(salinity, richness, fill = salinity)) +
  ...
  scale_fill_manual(values = salinity_palette_3, name = "Salinity") +
  labs(
...
# --- Generate per-site post-hoc letters for macro richness ---
lab_site <- div_df %>%
  group_split(site) %>%
  purrr::map_dfr(function(d) {
    ...
    out <- tryCatch(
      cld_letters(emmeans(m, ~ salinity)) %>% as_tibble(),
...
p_rich_all_treat <- ggplot(div_df, aes(treatment, richness, fill = treatment)) +
  ...
  geom_jitter(width = 0.15, alpha = 0.6, size = 1) +
  scale_fill_manual(values = salinity_palette_6, name = NULL) +   # no legend title
  labs(
    x = "Treatment",
    y = "Taxonomic richness",
...
p_rich_site_treat <- ggplot(div_df, aes(treatment, richness, fill = treatment)) +
  ...
  geom_jitter(width = 0.15, alpha = 0.6, size = 1) +
  facet_wrap(~ site) +
  scale_fill_manual(values = salinity_palette_6, name = NULL) +
...
# --- Mixed model for richness ---
m_bact_rich <- lmerTest::lmer(richness ~ salinity * nutrients + (1 | Site), data = bact_div_df)
...
# --- Mixed model for full factorial ---
m_bact_rich_treat <- lmerTest::lmer(richness ~ treatment + (1 | Site), data = bact_div_df)
...
########################################
# 1) MACRO – GLOBAL (Panel A)
########################################

dat_macro_global <- p_rich_all_treat$data %>%
  mutate(dummy = "Global")

pA_div <- ggplot(dat_macro_global, aes(treatment, richness, fill = treatment)) +
  geom_boxplot(alpha = 0.9, outlier.shape = NA) +
  geom_jitter(width = 0.15, alpha = 0.6, size = 1) +
  facet_wrap(~ dummy) +
  scale_fill_manual(values = salinity_palette_6, name = NULL) +
...
########################################
# 2) MACRO – PER SITE (Panel B)
########################################

pB_div <- p_rich_site_treat +
  labs(y = NULL, x = "Treatment", title = "") +
  theme(
...
########################################
# 3) BACTERIA – GLOBAL (Panel C)
########################################

dat_bact_global <- p_bact_rich_all_treat$data %>%
  mutate(dummy = "Global")

pC_div <- ggplot(dat_bact_global, aes(treatment, richness, fill = treatment)) +
  geom_boxplot(alpha = 0.9, outlier.shape = NA) +
  geom_jitter(width = 0.15, alpha = 0.6, size = 1) +
  facet_wrap(~ dummy) +
  scale_fill_manual(values = salinity_palette_6, name = NULL) +
...
########################################
# 4) BACTERIA – PER SITE (Panel D)
########################################

pD_div <- p_bact_rich_site_treat +
  labs(y = NULL, x = "Treatment", title = "") +
  theme(
    axis.title.y    = element_blank(),
    strip.background = element_rect(fill = "white", color = "black", linewidth = 0.6),
...
########################################
# 5) BUILD CORE 2×2 FIGURE (IDENTICAL TO MDS)
########################################

fig_core_diversity <- (
    pA_div + pB_div + plot_layout(widths = c(1, 3))
  ) /
  (
    pC_div + pD_div + plot_layout(widths = c(1, 3))
  ) +
...
########################################
# 6) ADD TITLES (IDENTICAL TEMPLATE)
########################################

title_macro_div <- ggplot() +
  annotate("text", x = 0.5, y = 0.5,
           label = "Macroinvertebrate richness",
           size = 6, fontface = "bold", hjust = 0) +
  theme_void()

title_bact_div <- ggplot() +
  annotate("text", x = 0.5, y = 0.5,
           label = "Benthic microbial richness",
           size = 6, fontface = "bold", hjust = 0) +
  theme_void()

fig_diversity_final <- 
  title_macro_div / fig_core_diversity[[1]] /
  title_bact_div  / fig_core_diversity[[2]] +
  plot_layout(heights = c(0.10, 1, 0.10, 1),
              guides = "collect") &
...
ggsave(
  filename = "figures/fig_combined_all_sites_FIXED.png",
  plot = fig_diversity_final,
  width = 12,
  height = 9,
  dpi = 400,
  bg = "white"
...
p_global <- ggplot(scores_df, aes(MDS1, MDS2, color = salinity)) +
  ...
  scale_color_manual(values = salinity_palette_3, name = "Salinity") +
...
p_global_treat <- ggplot(scores_df, aes(MDS1, MDS2, color = treatment)) +
  ...
  scale_color_manual(values = salinity_palette_6) +
  labs(title = "(A) All sites", x = "NMDS1", y = "NMDS2") +
  theme_classic(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold", size = 13, hjust = 0),
...
scores_macro_global <- scores_df %>%
  mutate(dummy = "Global")

pA <- ggplot(scores_macro_global, aes(MDS1, MDS2, color = treatment)) +
  geom_point(size = 2.3, alpha = 0.9) +
  stat_ellipse(aes(group = treatment),
               linetype = "dashed", linewidth = 0.7, level = 0.95) +
  facet_wrap(~ dummy, scales = "free") +
  scale_color_manual(values = salinity_palette_6, name = NULL) +
...
pB <- ggplot(scores_df, aes(MDS1, MDS2, color = treatment)) +
  geom_point(size = 2.3, alpha = 0.9) +
  stat_ellipse(aes(group = treatment),
               linetype = "dashed", linewidth = 0.7, level = 0.95) +
...
scores_bact_global <- scores_bact %>%
  mutate(dummy = "Global")

pC <- ggplot(scores_bact_global, aes(MDS1, MDS2, color = treatment)) +
  geom_point(size = 2.3, alpha = 0.9) +
  stat_ellipse(aes(group = treatment),
               linetype = "dashed", linewidth = 0.7, level = 0.95) +
  facet_wrap(~ dummy, scales = "free") +
...
pD <- ggplot(scores_bact, aes(MDS1, MDS2, color = treatment)) +
  geom_point(size = 2.3, alpha = 0.9) +
  stat_ellipse(aes(group = treatment),
               linetype = "dashed", linewidth = 0.7, level = 0.95) +
  facet_wrap(~ Site, scales = "free", ncol = 3) +
...
############################################################
# 5) COMBINE PANELS (this creates your existing working figure)
############################################################

library(patchwork)

fig_combined_mds <- (
    pA + pB + plot_layout(widths = c(1, 3))
  ) /
  (
    pC + pD + plot_layout(widths = c(1, 3))
  ) +
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A") &
  theme(
...
############################################################
# 6) ADD ROW TITLES (the missing final step)
############################################################

title_macro <- ggplot() +
  annotate("text", x = 0.5, y = 0.5,
           label = "Macroinvertebrate communities",
           size = 6, fontface = "bold") +
  theme_void()

title_bact <- ggplot() +
  annotate("text", x = 0.5, y = 0.5,
           label = "Benthic microbial communities",
           size = 6, fontface = "bold") +
  theme_void()

final_fig <- 
  title_macro / fig_combined_mds[[1]] /
  title_bact  / fig_combined_mds[[2]] +
...
ggsave(
  filename = "figures/fig_combined_mds.png",
  plot = final_fig,
  width = 12,
  height = 8,
  dpi = 400,
...
# =========================================================
# 2. Homogeneity of multivariate dispersion  (PERMDISP)
# =========================================================

bd_site <- betadisper(bray, df_meta$site)
bd_sal  <- betadisper(bray, df_meta$salinity)

disp_site <- permutest(bd_site, permutations = 999)
disp_sal  <- permutest(bd_sal, permutations = 999)
...
db_plot     <- db_plot     + labs(title = "Macroinvertebrate communities")
db_plot_b   <- db_plot_b   + labs(title = "Benthic microbial communities")

Fig_Macro_vs_Bacteria_dbRDA <- db_plot + db_plot_b +
  plot_layout(guides = "collect") +
...
Fig_Macro_vs_Bacteria_dbRDA

  ggsave(
    filename = "figures/Fig_Macro_vs_Bacteria_dbRDA.png",
    plot = Fig_Macro_vs_Bacteria_dbRDA,
    width = 10,
    height = 5,
    dpi = 400,
    bg = "white"
...
if (length(sig_taxa)) {
  ...
  fig5_Indicators <- ggplot(df_sel, aes(salinity, abundance, fill = salinity)) +
    ...
    scale_fill_manual(values = salinity_palette_3) +
    labs(
      x = "Salinity",
      y = "log1p(Abundance)",
...
if (length(sig_taxa_treat)) {
  ...
  figS3_Indicators <- ggplot(df_sel_treat, aes(treatment, abundance, fill = treatment)) +
    ...
    geom_jitter(width = 0.15, alpha = 0.6, size = 1) +
    facet_wrap(~ taxon, scales = "free_y") +
    scale_fill_manual(values = salinity_palette_6) +
    labs(
      x = "Treatment",
      y = "log1p(Abundance)",
...
m_grp <-lmerTest::lmer(log_abund ~ salinity + (1|site) + (1|group), data = df_long)
...
```
# Prepare datasets for PRIMER
```{r}
# ====== EXPORT FOR PRIMER: MACROINVERTEBRATES ======

# (A) COMMUNITY MATRICES
macro_comm_raw  <- comm_raw %>% rownames_to_column("SampleID")
macro_comm_log  <- comm_log %>% rownames_to_column("SampleID")
macro_comm_pa   <- comm_pa  %>% rownames_to_column("SampleID")

write_csv(macro_comm_raw, "data/PRIMER_exports/macro_comm_raw.csv")
write_csv(macro_comm_log, "data/PRIMER_exports/macro_comm_log1p.csv")
write_csv(macro_comm_pa,  "data/PRIMER_exports/macro_comm_pa.csv")

# (B) METADATA (ensure same order)
macro_meta_export <- df_meta %>%
  mutate(SampleID = row_number()) %>%   # matches row order of comm matrices
  select(SampleID, site, salinity, nutrients)

write_csv(macro_meta_export, "data/PRIMER_exports/macro_metadata.csv")

# ====== EXPORT FOR PRIMER: BACTERIA ======

# (A) COMMUNITY MATRICES
bact_comm_raw <- comm_bacteria %>%
  arrange(sample_code) %>%
  as.data.frame() %>%
  column_to_rownames("sample_code") %>%
  rownames_to_column("SampleID")

bact_comm_log <- bact_comm_raw %>%
  mutate(across(-SampleID, log1p))

bact_comm_pa <- bact_comm_raw %>%
  mutate(across(-SampleID, ~ ifelse(.x > 0, 1, 0)))

write_csv(bact_comm_raw, "data/PRIMER_exports/bact_comm_raw.csv")
write_csv(bact_comm_log, "data/PRIMER_exports/bact_comm_log1p.csv")
write_csv(bact_comm_pa,  "data/PRIMER_exports/bact_comm_pa.csv")

# (B) METADATA
bact_metadata_export <- df_bacteria_metadata %>%
  filter(sample_code %in% bact_comm_raw$SampleID) %>%
  rename(SampleID = sample_code) %>%
  select(SampleID, Site, salinity, nutrients, Treatment)

write_csv(bact_metadata_export, "data/PRIMER_exports/bacteria_metadata.csv")

# ===========================
# ENVIRONMENTAL DATA FOR PRIMER (MACROINVERTEBRATES)
# ===========================

write_csv(df_meta, "data/PRIMER_exports/df_meta.csv")


# Add SampleID aligned with community matrices
macro_env_export <- df_meta %>%
  mutate(SampleID = row_number()) %>%
  select(
    SampleID,
    site, salinity, nutrients,
    chlorophyll_a,
    pH,
...
# Add treatment (optional but recommended)
macro_env_export <- macro_env_export %>%
  mutate(
    treatment = case_when(
      salinity == "Ambient" & nutrients == "Ambient" ~ "ASAN",
      salinity == "Ambient" & nutrients == "High"    ~ "ASEN",
  ...
  relocate(treatment, .after = nutrients) %>% 
  as.data.frame()

# Export
write_csv(macro_env_export, "data/PRIMER_exports/macro_comm_raw.csv")
