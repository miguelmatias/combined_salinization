...
############################################################
# 1. FIX A (Macro – Global)
############################################################

dat_macro_global <- p_rich_all_treat$data %>%
  mutate(dummy = "Global")

pA <- ggplot(dat_macro_global, aes(treatment, richness, fill = treatment)) +
  geom_boxplot(alpha = 0.9, outlier.shape = NA) +
  geom_jitter(width = 0.15, alpha = 0.6, size = 1) +
  facet_wrap(~ dummy) +
  labs(title = "") +
...
############################################################
# 2. FIX C (Bacteria – Global)
############################################################

dat_bact_global <- p_bact_rich_all_treat$data %>%
  mutate(dummy = "Global")

pC <- ggplot(dat_bact_global, aes(treatment, richness, fill = treatment)) +
  geom_boxplot(alpha = 0.9, outlier.shape = NA) +
  geom_jitter(width = 0.15, alpha = 0.6, size = 1) +
  facet_wrap(~ dummy) +
  labs(title = "") +
...
############################################################
# 3. FIX B and D (remove y-axis labels)
############################################################

pB <- p_rich_site_treat +
  labs(title = "", y = NULL) +
  theme(
...
############################################################
# 4. COMBINE in a 2×2 layout with widths 1:3
############################################################

fig_combined_all_sites <- (
    pA + pB + 
...
fig_combined_all_sites
  ...
  ggsave(
    filename = "figures/fig_combined_all_sites.png",
    plot = fig_combined_all_sites,
    width = 12,
    height = 8,
    dpi = 400,
    bg = "white"
...
pA <- ggplot(scores_macro_global, aes(MDS1, MDS2, color = treatment)) +
  ...
  facet_wrap(~ dummy, scales = "free") +          # <<< FREE AXIS
...
pB <- ggplot(scores_df, aes(MDS1, MDS2, color = treatment)) +
  ...
  stat_ellipse(aes(group = treatment),
               linetype = "dashed", linewidth = 0.7, level = 0.95) +
  facet_wrap(~ site, scales = "free", ncol = 3) + # <<< FREE AXIS
  scale_color_manual(values = salinity_palette_6, name = NULL) +
  labs(x = "NMDS1", y = NULL) +
  theme_classic(base_size = 12) +
  theme(
...
############################################################
# 5) COMBINE 2×2 FIGURE (1:3 WIDTH PANELS)
############################################################
